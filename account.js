...
function showTab(id,button){
  ...
  const key=document.body.dataset.account;
  if(sessionStorage.getItem('zb_'+key)==='1'){
    if(key==='xinglover'&&id==='message')advance(1);
    if(key==='xingxing'&&id==='collect')advance(2);
    refreshGate(key);
  }
}
...
function unlockTeacherClue(){
  if(document.body.dataset.account!=='shangxin'||sessionStorage.getItem('zb_shangxin')!=='1'||progress()<2)return true;
  advance(3);
  return true;
}
...
document.addEventListener('DOMContentLoaded',()=>{const key=document.body.dataset.account,need=ORDER[key]||0;const b=document.querySelector('.login');if(progress()<need&&b)b.textContent='线索未解锁';unlock(key);refreshGate(key);if(key==='xinglover'){const a=document.querySelector('a[href="oldpage8.html"]');const meta=a&&a.closest('.item')&&a.closest('.item').querySelector('.meta');if(meta)meta.textContent='2023-06-10 · 一路向城关吧'}if(location.hash==='#drafts'&&key==='shangxin'&&sessionStorage.getItem('zb_shangxin')==='1'){const panel=document.getElementById('drafts'),tab=[...document.querySelectorAll('.tab')].find(x=>x.textContent.includes('草稿箱'));if(panel&&tab)showTab('drafts',tab)}});
